...
def lookforfails(runs,ofile):
  grepOut1 = subprocess.Popen(["grep","-rI","Exception","logs/"], stdout=subprocess.PIPE)
  out1, err = grepOut1.communicate()
  
  grepOut2 = subprocess.Popen(["grep","-rI","I think we have closure","logs/"], stdout=subprocess.PIPE)
...
def main():
  ...
  parser = argparse.ArgumentParser(description='Fetches crashed runs from stage4 and makes a runlist of failed runs.')
